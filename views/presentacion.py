...
class Presen(QMainWindow):
    ...
    def __init__(self):
        ...
        super(Presen, self).__init__()
        loadUi('C:\\Users\\andre\\OneDrive\\Escritorio\\Sistema-Hidrocolon-main\\views\\Presentacion.ui', self)
        self.por = 0
    ...
    def recibir(self):
        ...
        Presen._porcentaje = valor
        self.por = Presen._porcentaje
        self.switch_window.emit('ingresar_pre')
        self.lineEdit.clear()
        self.cargar_elementos()

    def recibir_eliminar(self):
        valor = self.comboBox.currentText()
        Presen._porcentaje_eliminar = valor
        self.switch_window.emit('eliminar_pre')
        print(f"{Presen._porcentaje_eliminar}")
        self.cargar_elementos()

    def recibir_editar(self):
        valor2 = self.cbox_agPresentacion_2.currentText()
        valor = self.lineEdit_3.text()
        Presen._porcentaje2 = valor
        Presen._porcentaje_com = valor2
        self.por = Presen._porcentaje
        self.switch_window.emit('editar_pre')
